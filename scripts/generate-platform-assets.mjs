...
function buildWorkflowExecutionPrompt(workflow, platform) {
  ...
  return [
    ...
    "8. Keep runtime boundaries explicit: use Playwright MCP for web flows, prefer `mobile-mcp` for semantic mobile flows, and switch to CLI fallback when you need deterministic logs, build state, or replayable evidence.",
    "",
    "Workflow source:",
    workflow.body.trim(),
    "",
...
function buildAgentExecutionPrompt(agent, platform) {
  ...
  return [
    ...
    "5. Keep runtime boundaries explicit: Playwright MCP for web, `mobile-mcp` first for semantic mobile work, CLI fallback for deterministic logs or replayable evidence.",
...
function buildClaudeHookScript() {
  return [
    ...
    "}",
    "",
    "if (/(^|[^a-z0-9-])(deep-research|design|web-ui-design|mobile-ui-design|design-system|skill-creator|api-design|database-design)([^a-z0-9-]|$)/i.test(normalized)) {",
    "  reminders.push(",
    ...
    "}",
    "",
    "if (/(web-testing|playwright|browser qa|browser test|website test|ui test)/i.test(normalized)) {",
    "  reminders.push(",
    '    "Web runtime detected. Prefer Playwright MCP for browser interaction and keep verification user-visible."',
    "  );",
    "}",
    "",
    "if (/(android|ios|mobile testing|mobile qa|emulator|simulator|mobile-mcp)/i.test(normalized)) {",
    "  reminders.push(",
    '    "Mobile runtime detected. Prefer mobile-mcp for semantic interaction and keep CLI fallback for deterministic logs, launch state, and replayable evidence."',
    "  );",
    "}",
    "",
...
function buildGeminiRouteResearchHookScript() {
  return [
    ...
    "",
    "if (/(web-testing|playwright|browser qa|browser test|website test|ui test)/i.test(normalized)) {",
    '  reminders.push("Web runtime detected. Prefer Playwright MCP for browser interaction and keep verification user-visible.");',
    "}",
    "",
    "if (/(android|ios|mobile testing|mobile qa|emulator|simulator|mobile-mcp)/i.test(normalized)) {",
    '  reminders.push("Mobile runtime detected. Prefer mobile-mcp for semantic interaction and keep CLI fallback for deterministic logs, launch state, and replayable evidence.");',
    "}",
    "",
    "const payload = reminders.length > 0",
    "  ? {",
