...
Canonical browser QA skill for live web flows. Use Playwright MCP first so the
runtime matches the real browser, the evidence is reproducible, and the verdict
is tied to actual page behavior rather than static code inspection.
...
3. Use stable selectors in this order: role, label, placeholder, test id, text, CSS last, because semantic locators survive refactors better than layout-driven selectors.
4. Inspect the page before you act, because snapshots and accessibility state often reveal the failure faster than blind clicking.
5. Capture evidence at key transitions, because screenshots, console output, and network data provide the proof behind the verdict.
6. Wait on concrete UI signals such as text, role state, URL changes, or response completion, because fixed sleeps are fragile and slow.
7. Keep the run focused on one flow at a time, because the goal is a defensible repro or verdict, not a giant exploratory script.
