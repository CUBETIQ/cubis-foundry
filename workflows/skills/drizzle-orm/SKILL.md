...
10. **Scope database connections to the runtime deployment model** because Cloudflare Workers use D1 or Hyperdrive bindings, Vercel Edge uses connection strings with `@vercel/postgres`, and Node.js uses `postgres` or `better-sqlite3`, each requiring different Drizzle driver initialization.
