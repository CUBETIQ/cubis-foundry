...
```javascript
// BEFORE - expensive stages too early
db.events.aggregate([
  {
    $lookup: {
      from: "users",
  ...
  { $match: { type: "click", createdAt: { $gte: ISODate("2024-01-01") } } },
]);

// AFTER - filter early, reduce data before lookup
db.events.aggregate([
  { $match: { type: "click", createdAt: { $gte: ISODate("2024-01-01") } } },
...
db.users.find({ active: true }, { email: 1, name: 1, _id: 0 }).limit(50);
```
