...
- `frontend-design` is the public front door. It locks the style contract, direction name, and thesis set before routing the work deeper.
...
1. Lock the style contract and the visual, content, and interaction theses.
2. Classify the request as routing/critique, systemization, web execution, mobile execution, or desktop execution.
3. If the interface already exists, diagnose the primary gap before proposing changes.
4. If the visual language needs to become durable, route to `design-system` before screen work.
5. If the task is a browser surface, route to `web-ui-design`.
6. If the task is a phone-first surface, route to `mobile-ui-design`.
7. If the task is a desktop-first workspace, route to `desktop-ui-design`.
8. Keep each surface inside its own contract. Do not blend critique, systemization, and execution in one answer.

## Canonical Design State

- `docs/foundation/DESIGN.md` is the canonical design-system artifact.
- Scoped overlays extend canonical design state only when the task needs narrower page, flow, or mobile detail.
- `.stitch/DESIGN.md` is a derived mirror for Stitch-facing flows, never the source of truth.

## Deliverable Contracts

### `frontend-design`

- State the direction name.
- State the style contract.
- State the visual, content, and interaction theses.
- Name the next artifact and the owning downstream surface.
...
- State the style contract and thesis set when they materially change downstream execution.
