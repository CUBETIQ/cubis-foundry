...
## Claude Stitch Compatibility Flow
...
- Verify the Foundry Stitch MCP configuration from `.mcp.json` before choosing any Stitch tool path.
- Prefer the shared `/implement` Stitch UI flow for new work so the design-first sequence stays intact on Claude.
