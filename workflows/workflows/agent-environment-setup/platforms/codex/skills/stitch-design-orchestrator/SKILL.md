...
- Treat Stitch as a rate-sensitive remote service on Codex: allow one generation or edit action per turn by default, prefer `edit_screens` over full regeneration, and stop after two automatic retries with backoff.
