...
- Keep TypeScript unit-test and type-test guidance in this skill instead of routing new work through a separate generic testing skill.
- Use the native TypeScript test stack already present in the repo, typically `vitest`, `expectTypeOf`, `tsd`, and framework-local helpers for code-level checks.
- Escalate API, persistence, browser, or other runtime-boundary checks into the owning framework or platform skill instead of a separate generic runtime-testing skill.
