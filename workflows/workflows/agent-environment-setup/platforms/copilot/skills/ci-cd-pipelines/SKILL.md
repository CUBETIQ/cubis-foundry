...
metadata:
  ...
  stack: [github-actions, gitlab-ci, jenkins, general-ci]
...
Design pipelines that are fast, safe, reproducible, and auditable.
...
- Monolithic pipeline files over 300 lines — split into reusable workflows and composite actions.
- Running full E2E suites on every PR — reserve for merge queue or staging.
- Storing secrets in workflow files — use repository or organization secrets.
- `continue-on-error: true` on critical checks — failures must block.
- Manual version bumps in CI — use semantic-release or similar automation.
- Running CI steps as root when not required.
