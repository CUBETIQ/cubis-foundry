...
11. For Stitch-style UI work, use this order: `design` -> `web-ui-design` or `mobile-ui-design` -> `frontend-design-system` only when canonical design state or the Stitch mirror is stale -> `stitch` only as a compat alias for older phrasing.
