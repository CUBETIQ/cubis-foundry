...
const DESIGN_FOUNDATION_BLOCK_START_RE =
  /<!--\s*cbx:design:foundation:start[^>]*-->/g;
const DESIGN_FOUNDATION_BLOCK_END_RE =
  /<!--\s*cbx:design:foundation:end\s*-->/g;
...
async function ensureArchitectureBuildScaffold({
  ...
  const designDir = path.join(workspaceRoot, FOUNDATION_DESIGN_DIR);
  const designPagesDir = path.join(designDir, "pages");
  const designFlowsDir = path.join(designDir, "flows");
  const designMobileDir = path.join(designDir, "mobile");
  const stitchDir = path.join(workspaceRoot, ".stitch");
  const stitchDesignPath = path.join(stitchDir, "DESIGN.md");
  ...
  const designDocResult = await upsertTaggedSectionInFile({
    targetPath: designDocPath,
    initialContent: `${buildDesignBuildSkeleton()}\n`,
    block: [
  ...
  if (!dryRun) {
    await mkdir(designPagesDir, { recursive: true });
    await mkdir(designFlowsDir, { recursive: true });
    await mkdir(designMobileDir, { recursive: true });
    await mkdir(stitchDir, { recursive: true });
    if (!(await pathExists(stitchDesignPath))) {
      await writeFile(
        stitchDesignPath,
        [
          "# Stitch Design Mirror",
          "",
          "This file mirrors `docs/foundation/DESIGN.md` plus any scoped design overlays when Stitch design flows are active.",
          "",
          "Do not edit this file directly. Regenerate it from the canonical design engine.",
          "",
        ].join("\n"),
        "utf8",
      );
...
async function configurePostmanInstallArtifacts({
  ...
  const legacySkillMcpCleanup =
    shouldInstallPostman && profilePaths.skillsDir
      ? await removeGeneratedArtifactIfExists({
...
function buildMcpPassthroughAliasName(service, toolName) {
...
function buildArchitecturePrompt({
  ...
  const designFlowsPath = `${FOUNDATION_DESIGN_DIR}/flows`;
  const designMobilePath = `${FOUNDATION_DESIGN_DIR}/mobile`;
  ...
  return [
    ...
    `- ${designPath} is the canonical design-system backbone for web, Stitch, and Flutter/mobile work. Use ${designPagesPath}, ${designFlowsPath}, and ${designMobilePath} for narrower overlays. Mirror Stitch-facing state into ${stitchDesignPath} only from the canonical docs.`,
...
async function readArchitectureDriftStatus(workspaceRoot, snapshot) {
  ...
  if (!designExists) {
    findings.push(`${FOUNDATION_DOCS_DIR}/DESIGN.md is missing.`);
  } else {
    const content = await readFile(designPath, "utf8");
    actualDesignHash = extractTaggedMarkerAttribute(
      content,
      DESIGN_FOUNDATION_BLOCK_START_RE,
      "profile",
    );
    if (!actualDesignHash) {
      findings.push(
        `${FOUNDATION_DOCS_DIR}/DESIGN.md is missing the managed design foundation block.`,
      );
    }
  }
  ...
  try {
    ...
    for (const service of ["postman", "stitch"]) {
      ...
      for (const tool of tools) {
        ...
        const aliasName = buildMcpPassthroughAliasName(service, name);
        if (
    ...
    for (const match of matches) {
      ...
      console.log(`  Alias: ${match.tool.aliasName}`);
      console.log(
