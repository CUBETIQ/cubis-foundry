...
**Available packs by language** (see [ruleset-catalog.md](../steering/ruleset-catalog.md)):
...
Threat models control which input sources CodeQL treats as tainted. The default (`remote`) covers HTTP/network input only. Expanding the threat model finds more vulnerabilities but may increase false positives. See [threat-models.md](../steering/threat-models.md) for details on each model.
...
If codebase is large then read [../steering/performance-tuning.md](../steering/performance-tuning.md) and apply relevant optimizations.
