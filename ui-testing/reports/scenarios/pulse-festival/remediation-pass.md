# pulse-festival Remediation Pass

- Style reference: `kinetic-dark-09`
- Style direction: `kinetic-poster`
...
### 1. design-audit
- Status: completed
- Triggered by: the hero shell cannot leave a dead page-level desktop column, the spotlight block and route rail must feel intentionally weighted against each other, lead with the current spotlight and route action before supporting pressure cards, keep route switching visible without forcing the full desktop shell to stack unchanged, the page feels like a designed poster with a real planner behind it, route switching updates route copy and the stage canvas, venue pressure remains visible as a decision cue rather than just decoration
...
- Actions:
  ...
  - Keep these desktop failure checks visible: the hero shell cannot leave a dead page-level desktop column; the spotlight block and route rail must feel intentionally weighted against each other.
