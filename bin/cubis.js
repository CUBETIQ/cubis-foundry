...
async function resolvePostmanInstallSelection({
  ...
  const apiKeySource = envApiKey ? "env" : "unset";
...
async function configurePostmanInstallArtifacts({
  ...
  const effectiveApiKeySource = envApiKey ? "env" : "unset";
...
function registerConfigKeysSubcommands(
  ...
  keysCommand
    ...
    .option("--dry-run", "preview changes without writing files")
    .action(wrap(runWorkflowConfigKeysMigrateInline));
